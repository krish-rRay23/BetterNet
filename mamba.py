...
from tensorflow.keras.layers import Layer, Conv1D, Dense, LayerNormalization, Activation
import math
...
class MambaLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        # 1D Convolution
        self.conv1d = Conv1D(filters=self.d_inner, kernel_size=self.d_conv, 
                             groups=self.d_inner, padding="causal", activation="swish")
    ...
    def call(self, x):
        ...
        A = -tf.exp(self.A_log) # shape: (d_inner, d_state)
        ...
        h_0 = tf.zeros((tf.shape(x)[0], self.d_inner, self.d_state))
