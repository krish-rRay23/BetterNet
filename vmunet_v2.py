...
class SDIModule(tf.keras.layers.Layer):
    ...
    def call(self, encoder_feat, decoder_feat):
        h, w = tf.shape(encoder_feat)[1], tf.shape(encoder_feat)[2]
        decoder_feat_resized = Resizing(h, w, interpolation='bilinear')(decoder_feat)
        fused = Concatenate()([encoder_feat, decoder_feat_resized])
        fused = self.conv_fuse(fused)
        fused = self.cbam(fused)
        return fused

def vmunet_v2(input_shape=(256, 256, 3), num_classes=1, base_filters=32, use_freq_mamba=True):
    """
    ...
    # --- Encoder (VSS Blocks) ---
    x1 = Conv2D(base_filters, (3, 3), padding='same', strides=2)(inputs)
